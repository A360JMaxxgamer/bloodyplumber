...

{
    public class Level 
    {
        ...
        public void Initialize(Texture2D pixel,int rows, int columns, Vector2 screenSolution)
        {
            m_pixel = pixel;
            m_rows = rows;
            m_columns = columns;
        ...
        public void addTile(int xPosition, int yPosition, Tile currentTile)
        {
            ...
            for( int i = 0; i< m_listOfTiles.Count; i++)
            {
                if(m_tilesWidth*x == m_listOfTiles.ElementAt(i).getPosition().X && m_tilesHeight* y ==m_listOfTiles.ElementAt(i).getPosition().Y)
                {
            ...
            {
                Tile tmp = new Tile();
                tmp.Initialize(currentTile.getImage(), currentTile.getXStart(), currentTile.getYStart(), currentTile.getTileWidth(), currentTile.getTileHeight(), position.X, position.Y, currentTile.getScale(),currentTile.getNumber());
                tmp.setPosition(position);
                m_listOfTiles.Add(tmp);
            }
        }

        public void addEnemy(int xPosition, int yPosition, Enemy currentEnemy)
        {
            int x = xPosition / m_tilesWidth;
            int y = yPosition / m_tilesHeight;
            Vector2 position = new Vector2(x*m_tilesWidth,y*m_tilesHeight);
            ...
            if (!replaced)
            {
                ...
                tmpAnimation.Initialize(currentEnemy.getAnimation().getAnimationStrip(), position.X, position.Y, currentEnemy.getAnimation().getFrameWidth(), currentEnemy.getAnimation().getFrameWidth(), currentEnemy.getAnimation().getFrameCount(), currentEnemy.getAnimation().getFrameTime(),true, new Vector2(1,1));
                tmp.Initialize(position.X, position.Y, currentEnemy.f_xVelocity, currentEnemy.f_xVelocity, currentEnemy.f_Speed, currentEnemy.m_Health, tmpAnimation, 1920);
                m_listOfEnemies.Add(tmp);
            }
        }
